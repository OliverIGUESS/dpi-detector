...
```bash
# Создайте нужные файлы: domains.txt, tcp_16_20_targets.json или config.py
...
domains.txt
# Отредактируйте tcp_16_20_targets.json для добавления своих доменов TCP 16-20KB блокировки
tcp_16_20_targets.json
# Настройте параметры в config.py (таймауты, количество потоков и т.д.)
config.py
```

## ⚙️ Конфигурация

Откройте `dpi_detector.py` и настройте параметры в начале файла:
```python
# === Основные настройки ===
USE_IPV4_ONLY = True              # Использовать только IPv4 (отключить IPv6)
MAX_CONCURRENT = 150              # Количество параллельных проверок

# === Таймауты ===
TIMEOUT = 6.0                     # Таймаут для проверки доменов (сек)
TIMEOUT_TCP_16_20 = 12.0          # Таймаут для TCP 16-20KB теста (сек)

# === Повторные попытки ===
DOMAIN_CHECK_RETRIES = 1          # Количество попыток для доменов
TCP_16_20_CHECK_RETRIES = 1       # Количество попыток для TCP теста

# === TCP блокировка ===
TCP_BLOCK_MIN_KB = 4              # Минимальный порог детекта TCP блокировки (KB)
TCP_BLOCK_MAX_KB = 60             # Максимальный порог детекта TCP блокировки (KB)
```
